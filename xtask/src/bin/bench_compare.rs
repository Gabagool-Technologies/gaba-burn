...
fn main() -> anyhow::Result<()> {
    ...
    let baseline = env::var("BENCH_BASELINE_CSV")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("crates/gaba-native-kernels/benches/baseline.csv"));
    let current = env::var("BENCH_CURRENT_CSV")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("target/bench-csv/gaba-native-kernels.csv"));

    let threshold: f64 = env::var("BENCH_MAX_REGRESSION_PCT")
        .ok()
    ...
    for (name, base_val) in &baseline_map {
        if let Some(curr_val) = current_map.get(name) {
            ...
            println!(
                "{}: baseline={}s current={}s change={:.2}%",
                name, base_val, curr_val, pct
            );
            let pass = pct <= threshold;
            if !pass {
                println!(
                    "REGRESSION: {} changed by >{:.2}% ({:.2}%)",
                    name, threshold, pct
                );
                failed = true;
            }
