...
use crate::{
    encode_cyclical, InferenceError, Result, RoadSegment, Season, TrafficFeatures, WeatherCondition,
};
...
impl TrafficPredictor {
    ...
    pub fn predict(&mut self, features: &TrafficFeatures) -> Result<f32> {
        ...
        // Extract prediction
        let output_tensor = outputs["speed"].try_extract_array::<f32>()?;
    ...
    fn encode_features(&self, features: &TrafficFeatures) -> Vec<f32> {
        ...
        vec.push(if features.weather == WeatherCondition::HeavyRain {
            1.0
        } else {
            0.0
        });
        ...
        vec.push(if features.weather == WeatherCondition::HeavySnow {
            1.0
        } else {
            0.0
        });
        vec.push(if features.weather == WeatherCondition::Fog {
            1.0
        } else {
            0.0
        });
        ...
        vec.push(if features.road_segment == RoadSegment::GspExit83_90 {
            1.0
        } else {
            0.0
        });
        vec.push(if features.road_segment == RoadSegment::GspExit117_120 {
            1.0
        } else {
            0.0
        });
        vec.push(if features.road_segment == RoadSegment::GspExit29_30 {
            1.0
        } else {
            0.0
        });
        vec.push(if features.road_segment == RoadSegment::TurnpikeI95 {
            1.0
        } else {
            0.0
        });
        vec.push(if features.road_segment == RoadSegment::Route1 {
            1.0
        } else {
            0.0
        });
        vec.push(if features.road_segment == RoadSegment::Route9 {
            1.0
        } else {
            0.0
        });
