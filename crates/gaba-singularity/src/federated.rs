...
impl FederatedLearningEngine {
    ...
    pub fn apply_differential_privacy(&self, vectors: Vec<PerformanceVector>) -> Vec<PrivatePerformanceVector> {
    ...
    pub fn aggregate_knowledge(&self, private_vectors: Vec<PrivatePerformanceVector>) -> Vec<AggregatedKnowledge> {
        ...
        for pv in private_vectors {
            ...
            aggregates.entry(key).or_insert_with(Vec::new).push(pv.execution_time_ms);
        }
        
        aggregates.into_iter().map(|((m, n, k, kernel_type), times)| {
            let median = {
