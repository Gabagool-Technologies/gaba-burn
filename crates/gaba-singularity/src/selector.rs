...
impl KernelSelector {
    ...
    pub fn predict(&mut self, features: &WorkloadFeatures, 
                   similar_history: &[PerformanceVector]) -> KernelType {
        let state_hash = features.signature();
        
        if !similar_history.is_empty() && rand::random::<f64>() > self.epsilon {
            return similar_history.iter()
        ...
        KernelType::ALL.iter()
            .max_by_key(|kernel| {
                let q_value = self.q_table.get(&(state_hash, **kernel))
                    .unwrap_or(&0.0);
    ...
    pub fn update_q_value(&mut self, features: &WorkloadFeatures, kernel: KernelType, 
                         execution_time: std::time::Duration) {
        let state_hash = features.signature();
        
        let reward = 1.0 / execution_time.as_secs_f64().max(0.000001);
        
        let current_q = self.q_table.get(&(state_hash, kernel))
            .unwrap_or(&0.0);
        
        let max_next_q = KernelType::ALL.iter()
