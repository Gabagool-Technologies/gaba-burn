...
pub fn solve_tsp_nearest_neighbor(
    ...
    #[cfg(feature = "zig")]
    let distance =
        unsafe { tsp_nearest_neighbor(distance_matrix.as_ptr(), n, start_idx, route.as_mut_ptr()) };
...
#[cfg(not(feature = "zig"))]
fn tsp_two_opt_rust(distance_matrix: &[f64], route: &mut [usize], max_iterations: usize) -> f64 {
    let n = route.len();
    if n < 4 {
