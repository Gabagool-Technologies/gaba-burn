...
pub fn transformer_block_forward(
    ...
    let attn_out = multi_head_attention_forward(
        input, input, input,
        &params.attention,
        seq_len,
        embed_dim,
    ...
    // Layer norm 1
    let ln1_out = layer_norm(&residual1, seq_len, embed_dim, &params.ln1_gamma, &params.ln1_beta);
    ...
    // Layer norm 2
    layer_norm(&residual2, seq_len, embed_dim, &params.ln2_gamma, &params.ln2_beta)
}
...
fn layer_norm(input: &[f32], seq_len: usize, embed_dim: usize, gamma: &[f32], beta: &[f32]) -> Vec<f32> {
...
mod tests {
    ...
    fn test_multi_head_attention() {
        ...
        let output = multi_head_attention_forward(&input, &input, &input, &params, seq_len, embed_dim);
