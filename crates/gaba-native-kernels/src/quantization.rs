...
impl QuantizationParams {
    pub fn from_range(min: f32, max: f32) -> Self {
        ...
        let zero_point = if scale > 0.0 { (-min / scale).round().clamp(-128.0, 127.0) as i8 } else { 0 };
...
"#;
    ...
    let kernel = library.get_function("gemm_i8_kernel", None)
...
mod tests {
    ...
    fn test_quantization_roundtrip() {
        ...
        for (i, &val) in input.iter().enumerate() {
            ...
            assert!(relative_error < 0.25, "Value {} vs {} (error: {}, scale: {})", output[i], val, error, params.scale);
        }
    }
