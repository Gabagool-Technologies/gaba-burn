...
impl LSTMParams {
    pub fn new(input_size: usize, hidden_size: usize) -> Self {
        ...
        let init_bias = |size: usize| -> Vec<f32> { vec![0.0; size] };
...
mod tests {
    ...
    fn test_lstm_forward() {
        ...
        let input_sequence: Vec<Vec<f32>> = (0..seq_len).map(|_| vec![1.0; input_size]).collect();

        let mut output_sequence: Vec<Vec<f32>> =
            (0..seq_len).map(|_| vec![0.0; hidden_size]).collect();
