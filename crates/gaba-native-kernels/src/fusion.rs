...
pub fn gemm_relu_fused(a: &[f32], b: &[f32], c: &mut [f32], m: usize, n: usize, k: usize) -> Duration {
...
pub fn gemm_relu_vectorized(a: &[f32], b: &[f32], c: &mut [f32], m: usize, n: usize, k: usize) -> Duration {
    let start = std::time::Instant::now();
...
pub fn gemm_relu_accelerate(a: &[f32], b: &[f32], c: &mut [f32], m: usize, n: usize, k: usize) -> Duration {
...
"#;
    ...
    let kernel = library.get_function("gemm_relu_kernel", None)
...
mod tests {
    ...
    fn test_gemm_relu_fused() {
        ...
        assert!(c.iter().all(|&v| v >= 0.0), "All values should be >= 0 after ReLU");
    }
