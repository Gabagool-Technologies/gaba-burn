...
impl AdamOptimizer {
    ...
    pub fn step(&mut self, params: &mut HashMap<String, Vec<f32>>, grads: &HashMap<String, Vec<f32>>) {
        self.t += 1;
        ...
        for (name, param) in params.iter_mut() {
            ...
            let m = self.m.entry(name.clone()).or_insert_with(|| vec![0.0; param.len()]);
            let v = self.v.entry(name.clone()).or_insert_with(|| vec![0.0; param.len()]);
    ...
    pub fn step(&mut self, params: &mut HashMap<String, Vec<f32>>, grads: &HashMap<String, Vec<f32>>) {
        ...
        for (name, param) in params.iter_mut() {
            ...
            for i in 0..param.len() {
                ...
                param[i] -= self.lr * (m_hat / (v_hat.sqrt() + self.epsilon) + self.weight_decay * param[i]);
            }
        }
    }
}
